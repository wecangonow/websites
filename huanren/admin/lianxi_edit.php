...
if($_REQUEST["action"]=="edit")
{
    ...
    $condition="id=".$id;
    $dataArray = array("cat_id"=>$cat_id,"title"=>$title,"tel"=>$tel,"picture"=>$picture,"file_url"=>$file_url,"laiyuan"=>$laiyuan,"tags"=>$tags,"sort_order"=>$sort_order,"note"=>$note,"content"=>$content,"add_time"=>$add_time);
...
?>
    <!-- tab body -->
    <div id="tabbody-div">
      <form action="lianxi_edit.php?action=edit&id=<?php echo $result['id']; ?>&L_parent_id=<?php echo $L_parent_id; ?>&L_cat_id=<?php echo $L_cat_id; ?>&L_keywords=<?php echo $L_keywords; ?>&L_page=<?php echo $L_page; ?>" method="post" name="theForm" onsubmit="return check_submit();">
