...
if($_REQUEST["action"]=="edit")
{
    ...
    $condition="id=".$id;
    $dataArray = array("cat_id"=>$cat_id,"title"=>$title,"is_show"=>$is_show,"picture"=>$picture,"file_url"=>$file_url,"laiyuan"=>$laiyuan,"tags"=>$tags,"sort_order"=>$sort_order,"note"=>$note,"content"=>$content,"add_time"=>$add_time);
...
?>
    <!-- tab body -->
    <div id="tabbody-div">
      <form action="product_edit.php?action=edit&id=<?php echo $result['id']; ?>&L_parent_id=<?php echo $L_parent_id; ?>&L_cat_id=<?php echo $L_cat_id; ?>&L_keywords=<?php echo $L_keywords; ?>&L_page=<?php echo $L_page; ?>" method="post" name="theForm" onsubmit="return check_submit();">
        <table width="90%" id="general-table" align="center">
          ...
          <tr>
            ...
            <tr>
                <td width="17%" class="label">是否推荐</td>
                <td width="83%">
                    <input type="radio" id="yes" <?php if($result["is_show"] == 1){?> checked="checked" <?php }?> name="is_show"  value="1" />
                    <label for="yes">是</label>
                    <input type="radio" id="no"  <?php if($result["is_show"] == 0){?> checked="checked" <?php }?> name="is_show"  value="0" />
