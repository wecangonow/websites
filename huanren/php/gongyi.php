...
$sql="select * from pic where id=15";
...
?>
		<div class="qing cai">
            <div class="qing">
                <div class="lf cai_bt">
                    <a href="gongyi_view.php?id=<?php echo $row_list["id"];?>" class="qing"><?php echo sub_str($row_list["title"],40);?></a>
                    <span class="qing"><?php echo date("Y/m/d",strtotime($row_list["add_time"])); ?></span>
                </div>
                <a href="gongyi_view.php?id=<?php echo $row_list["id"];?>" class="cm1"<?php if($i==1) {?> style="top:142px; left:0; border:2px solid #4090cd; color:#005ca2;"<?php }?>>查看详细</a>
            </div>
            <div class="qing cai_shao"<?php if($i==1) {?> style=" height:110px;"<?php }?>>
                <div class="qing cai_tiao"></div>
                <a href="gongyi_view.php?id=<?php echo $row_list["id"];?>" class="qing cai_jian"><?php echo sub_str($row_list["note"],100);?></a>
            </div>
        </div>
<?php
$i=$i+1;
