...
$sql="select * from news where cat_id=7 and id>".$news_view["id"]." order by id asc limit 0,1";
...
{
    $prev_info=array('title'=>$prev_info['title'],'url'=>'model_view.php?id='.$prev_info['id']);
}

$sql="select * from news where cat_id=7 and id<".$news_view["id"]." order by id desc limit 0,1";
...
{
    $next_info=array('title'=>$next_info['title'],'url'=>'model_view.php?id='.$next_info['id']);
}
?>
...
<div class="qing center" style="padding-bottom:44px; padding-top:23px;">
	<div class="qing xw">
		...
		<div class="qing return">
        	<a href="model.php" class="lf"><span class="qing re1">返回列表</span><span class="qing re2">返回列表</span></a>
        </div>
	</div>
</div>
<!-- 底部 -->
