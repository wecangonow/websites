...
if($_REQUEST["action"]=="edit")
{
    ...
    $condition="id=".$id;
    $dataArray = array("title"=>$title,"picture"=>$picture,"text_picture"=>$text_picture,"note"=>$note,"content"=>$content,"add_time"=>$add_time);
	$mysql->update("news",$dataArray,$condition);
	echo '<script>alert("操作成功");location.href="zongcai_info.php?id='.$id.'";</script>';
	exit;
}
...
?>
    <!-- tab body -->
    <div id="tabbody-div">
      <form action="zongcai_info.php?action=edit&id=<?php echo $id; ?>" method="post" name="theForm" onsubmit="return check_submit();">
        <table width="90%" id="general-table" align="center">
          ...
          <tr>
            ...
            <td><input type="text" name="picture" id="picture" class="input_text" value="<?php echo $result['picture']; ?>" />&nbsp;<input name="upfile" type="button" class="button" onClick="javascript:opw('includes/pic_upload_form.php?text_id=picture&saveTo=../../upload/images/&showPath=upload/images/','picture',500,250)" value="上传图片"> (290*387)</td>
          </tr>
          <tr>
            <td class="label">首页图片</td>
            <td><input type="text" name="text_picture" id="text_picture" class="input_text" value="<?php echo $result['text_picture']; ?>" />&nbsp;<input name="upfile" type="button" class="button" onClick="javascript:opw('includes/pic_upload_form.php?text_id=text_picture&saveTo=../../upload/images/&showPath=upload/images/','text_picture',500,250)" value="上传图片"> (634*313)</td>
          </tr>
