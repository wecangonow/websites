...
if ($_REQUEST['action'] == 'delete')
{
	...
	$condition="id=".$id;
	$mysql->delete("baoming",$condition);
	echo '<script>alert("操作成功");location.href="baoming_list.php?keywords='.$keywords.'&page='.$page.'";</script>';
	exit;
}
elseif ($_REQUEST['action'] == 'batch')
{
	...
	elseif($_REQUEST["types"]=='is_show')
	{
		...
		$mysql->update("baoming",$dataArray,$condition);
	}
...
<!-- 文章列表 -->
<form method="post" action="baoming_list.php?keywords=<?php echo $keywords; ?>&page=<?php echo $page; ?>" name="listForm" onsubmit="return confirmSubmit(this)">
  <!-- start goods list -->
...
<table width="1103" cellpadding="3" cellspacing="1">
  <tr>
    ...
    <th width="59">ID</th>
    <th width="134">培训人名</th>
    <th width="108">所在地区</th>
    <th width="215">所在学校</th>
    <th width="128">电子邮箱</th>
    <th width="131">联系方式</th>
    <th width="160">报名时间</th>
    <th width="60">操作</th>
  <tr>
<?php 
$maxnum = 50;  //每页显示记录条数
$sql = "SELECT * FROM baoming where 1=1";
if($keywords!='')
{
	$sql = $sql." and (kecheng_title like '%$keywords%' or username like '%$keywords%' )";
}
...
?>
  <tr>
    ...
    <td class="first-cell" style=""><a href="baoming_view.php?id=<?php echo $result['id']; ?>"><?php echo $result['username']; ?></a></td>
    ...
    <td align="center">
     <a href="baoming_list.php?action=delete&id=<?php echo $result['id']; ?>" title="" onClick="return confirm('您确定进行删除操作吗？')"><img src="images/icon_trash.gif" width="16" height="16" border="0" /></a>
    </td>
  </tr>
<?php
}
$gotoPageFirst = "baoming_list.php?keywords=".$keywords."&page=1";
$gotoPagePrev = "baoming_list.php?keywords=".$keywords."&page=".($page-1);
$gotoPageNext = "baoming_list.php?keywords=".$keywords."&page=".($page-1);
$gotoPageFirst = "baoming_list.php?keywords=".$keywords."&page=".$totalpages;
?>
</table>
<!-- end goods list -->
...
function gotopages()
{
	window.location.href='baoming_list.php?keywords='+$("#keywords").val()+'&page='+$("#gotoPage").val();
}
