...
if ($_REQUEST['action'] == 'delete')
{
	...
	echo '<script>alert("操作成功");location.href="gongyicishan_list.php?cat_id='.$L_cat_id.'&keywords='.$L_keywords.'&page='.$L_page.'";</script>';
	exit;
}
...
<!-- 文章列表 -->
<form method="post" action="gongyicishan_list.php?L_cat_id=<?php echo $cat_id; ?>&L_keywords=<?php echo $keywords; ?>&L_page=<?php echo $page; ?>" name="listForm" onsubmit="return confirmSubmit(this)">
  <!-- start goods list -->
  <div class="list-div" id="listDiv">
<table width="1103" cellpadding="3" cellspacing="1">
  <tr>
    <th width="34"><input name="checkall" id="checkall" type="checkbox" /></th>
    <th width="58">ID</th>
    <th width="375">标题</th>
    <th width="118">所属分类</th>
    <th width="94">首页推荐</th>
    <th width="94">列表推荐</th>
    <th width="55">排序</th>
    <th width="153">更新时间</th>
    <th width="56">操作</th>
  <tr>
<?php 
$maxnum = 50;  //每页显示记录条数
...
?>
  <tr>
    ...
    <td class="first-cell" style=""><a href="gongyicishan_edit.php?id=<?php echo $result['id']; ?>&L_cat_id=<?php echo $cat_id; ?>&L_keywords=<?php echo $keywords; ?>&L_page=<?php echo $page; ?>"><?php echo $result['title']; ?></a></td>
    ...
    <td align="center">
      <a href="gongyicishan_edit.php?id=<?php echo $result['id']; ?>&L_cat_id=<?php echo $cat_id; ?>&L_keywords=<?php echo $keywords; ?>&L_page=<?php echo $page; ?>" title=""><img src="images/icon_edit.gif" width="16" height="16" border="0" /></a>
      <a href="gongyicishan_list.php?action=delete&id=<?php echo $result['id']; ?>&L_cat_id=<?php echo $cat_id; ?>&L_keywords=<?php echo $keywords; ?>&L_page=<?php echo $page; ?>" title="" onClick="return confirm('您确定进行删除操作吗？')"><img src="images/icon_trash.gif" width="16" height="16" border="0" /></a>
    </td>
  </tr>
<?php
}
$gotoPageFirst = "gongyicishan_list.php?cat_id=".$cat_id."&keywords=".$keywords."&page=1";
$gotoPagePrev = "gongyicishan_list.php?cat_id=".$cat_id."&keywords=".$keywords."&page=".($page-1);
$gotoPageNext = "gongyicishan_list.php?cat_id=".$cat_id."&keywords=".$keywords."&page=".($page-1);
$gotoPageFirst = "gongyicishan_list.php?cat_id=".$cat_id."&keywords=".$keywords."&page=".$totalpages;
?>
</table>
<!-- end goods list -->
...
<div>
  ...
  <select id="types" name="types" onchange="changeAction()">
    ...
    <option value="is_show">设置首页推荐</option>
    <option value="not_is_show">取消首页推荐</option>
    <option value="is_list">设置列表推荐</option>
    <option value="not_is_list">取消列表推荐</option>
...
function gotopages()
{
	window.location.href='gongyicishan_list.php?cat_id=<?php echo $cat_id; ?>&keywords='+$("#keywords").val()+'&page='+$("#gotoPage").val();
}
